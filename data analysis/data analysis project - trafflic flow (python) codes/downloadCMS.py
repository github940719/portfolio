...
def download_one_file(y, month, d, h, min, gz_file_path, xml_file_path):
    ...
    # there are data only every 2 mins
    if (min % 2 != 0):  # the min is odd -> no data
        return

    # request response from the url
    url = "https://tisvcloud.freeway.gov.tw/history/motc20/CMS/"
    url_file = f"{y}{month:02}{d:02}/CMSLive_{h:02}{min:02}.xml.gz"
    response = requests.get(url + url_file)

    # set the gz_file path and name
    gz_file_name = f"CMSLive_{y}{month:02}{d:02}_{h:02}{min:02}.xml.gz"
    ...
    xml_file_name = f"CMSLive_{y}{month:02}{d:02}_{h:02}{min:02}.xml"
...
if __name__ == "__main__":
    ...
    gz_file_path = \
        "C:/Users/jaspe/Desktop/課程/資料分析/data_analysis_module3/CMS_gz/"
    xml_file_path = \
